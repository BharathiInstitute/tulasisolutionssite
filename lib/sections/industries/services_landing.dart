...
class _FeaturesGrid extends StatelessWidget { const _FeaturesGrid({required this.items}); final List<FeatureItem> items; @override Widget build(BuildContext context) { return LayoutBuilder(builder: (context, c) { final w = c.maxWidth; int cols = 1; if (w >= 1024) { cols = 4; } else if (w >= 700) { cols = 2; } return GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: items.length, gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: cols, mainAxisExtent: 150, crossAxisSpacing: 16, mainAxisSpacing: 16), itemBuilder: (context, i) => _FeatureCard(item: items[i], index: i)); }); } }
...
ServicesLandingContent sampleServicesContent({required VoidCallback onPrimary, required VoidCallback onSecondary,}) {
	return ServicesLandingContent(
		...
		quickChips: const [ChipItem('👥 Persona packs'), ChipItem('🌐 Website layouts'), ChipItem('📋 SOPs'), ChipItem('📊 KPIs')],
		features: const [FeatureItem('👤', 'Persona packs', 'Freelancers, SMBs, corporates'), FeatureItem('🌐', 'Website layouts', 'Pricing, testimonials, catalogs'), FeatureItem('📋', 'SOPs', 'Onboarding & ticketing'), FeatureItem('📊', 'KPIs', 'Client retention, upsells')],
		onPrimary: onPrimary,
